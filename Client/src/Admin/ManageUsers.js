...
function ManageUsers() {
    ...
    useEffect(() => {
        const fetchUsers = async () => {
            try {
                setLoading(true);
                const response = await axios.post('http://localhost:5000/api/admin/fetch-users', { role: 'user' });
    ...
    const handleConfirmAction = async () => {
        try {
            if (actionType === 'ban') {
                ...
                await axios.post('http://localhost:5000/api/admin/unban-user', { userId: selectedUser._id });
            ...
            console.error(`Error during ${actionType}:`, err.response?.data || err.message);
            setError(err.response?.data?.message || `Failed to ${actionType} user`);
            toast.error(err.response?.data?.message || `Error during ${actionType} action`);
        } finally {
            setReason('');
            setBanDuration('');
                                ...
                                <div className="modal-body manage-users-modal-body">
                                    {actionType === 'unban' ? (
                                        ...
                                        <>
                                            ...
                                            {actionType === 'ban' && (
                                                <div className="mt-3">
                                                    <label htmlFor="banDuration">Ban Duration (in minutes):</label>
                                                    <input
                                                        type="number"
                                ...
                                <div className="modal-footer manage-users-modal-footer">
                                    ...
                                    <button
                                        ...
                                        disabled={
                                            actionType === 'unban'
                                                ? false
                                                : !reason.trim() || (actionType === 'ban' && !banDuration.trim())
                                        }
                                    >
                                        Confirm
                                    </button>
...
export default withAutoLogout(withAuthProtection(withRoleProtection(ManageUsers, ['admin'])));
